...
confirming, by hand, that the game loads the pack and that running
`example:raw_notes` prints `first line` then `second line`. Only then does this
case count as passing (ROADMAP Phase 0 exit criteria).
...
- Confirm the `function` category directory and extension for 26.2 from target
  data.
- Confirm the emitted function file is `source` verbatim, including the comment,
  the blank line, and the trailing newline.

Expected contents (shape only, to be filled in once verified): `pack.mcmeta`, and
one function file for `example:raw_notes` equal to the `source` string.
